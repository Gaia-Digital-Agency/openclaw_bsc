...
- `Parent Last Name` OR `Family or Group name` (legacy — still accepted)
- `Parent first name`
- `Email`
- `Password`
...
Student Last Name (optional, blank uses parent last name):
First name *:
Gender (M/F) *:
Birthday (DD/MM/YYYY) *:
...
**Parent fields:**
- `parentLastName` ← Parent Last Name (accept legacy label `Family or Group name`)
...
- `youngsterLastName` ← Student Last Name (OPTIONAL — omit from payload if blank; API will default to parent last name)
...
- `youngsterGrade` ← Grade (valid values: PS, PR, Pre K, G1..G12 — case-insensitive, normalise to canonical label)
...
**Grade (per student):**
- Must be one of: `PS`, `PR`, `Pre K`, `G1`..`G12` (case-insensitive, normalise e.g. `prek` / `pre-k` → `Pre K`, `g1` → `G1`)
- Error: `🎓 Student {N} grade not recognised — use PS, PR, Pre K, or G1-G12`

**Student Last Name (per student):**
- Optional. If provided, must be 1–100 characters and contain no digits or control characters.
- Error (only if provided and invalid): `⚠️ Student {N} last name is invalid — max 100 characters, letters only`
...
🎓 Student 1 — {youngsterFirstName} {youngsterLastName || parentLastName}
   Gender: {Male/Female}
...
{repeat for each student — use the student's own last name when provided, otherwise fall back to parentLastName}
